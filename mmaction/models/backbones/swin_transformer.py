...
def window_reverse(windows, window_size, B, D, H, W):
    ...
    Returns:
        x: (B, D, H, W, C)
    """
    x = windows.view(B, D // window_size[0], H // window_size[1], W //
                     window_size[2], window_size[0], window_size[1], window_size[2], -1)
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        x_windows = window_partition(
            shifted_x, window_size)  # B*nW, Wd*Wh*Ww, C
        # W-MSA/SW-MSA
        ...
        shifted_x = window_reverse(
            attn_windows, window_size, B, Dp, Hp, Wp)  # B D' H' W' C
...
def compute_mask(D, H, W, window_size, shift_size, device):
    ...
    mask_windows = window_partition(
        img_mask, window_size)  # nW, ws[0]*ws[1]*ws[2], 1
...
#        shortcut = x
        ...
        for blk in self.blocks:
            x = blk(x, attn_mask)
        x = x.view(B, D, H, W, -1)
#        shortcut = shortcut.view(B, D, H, W, -1)
#        x = x+shortcut
...
class SwinTransformer3D(nn.Module):
    ...
    def inflate_weights(self, logger):
        ...
        state_dict['patch_embed.proj.weight'] = state_dict['patch_embed.proj.weight'].unsqueeze(
            2).repeat(1, 1, self.patch_size[0], 1, 1) / self.patch_size[0]
