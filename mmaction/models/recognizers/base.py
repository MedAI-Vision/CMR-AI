...
#from .densenet_av import densenet_40_12_bc
import cv2
import numpy as np
#import sys
# sys.path.append('/home/yanran/VST/mmaction/models/recognizers')
#from feature_hook import get_fea_by_hook
    ...
    def __init__(self,
        ...
        if fusion:
            ...
            if sax_weight is not None:
                self.weight.append(sax_weight)
            if ch_weight is not None:
                self.weight.append(ch_weight)
            if lge_weight is not None:
                self.weight.append(lge_weight)
...
#                get_fea_by_hook(self.backbone)
            else:
                self.backbone = nn.ModuleList()
                for weight in self.weight:
                    backbone_cache = builder.build_backbone(backbone)
                    backbone_cache.load_state_dict(
                        torch.load(weight, map_location='cpu'))
                    for p in backbone_cache.parameters():
                        p.requires_grad = False
                        
                    self.backbone.append(backbone_cache)
        if neck is not None:
    ...
    def extract_feat(self, imgs):
        ...
        """
        if self.fusion is False:
            #            plt.imshow(imgs[1,0,0,:,:].cpu(),cmap='gray')
            #            plt.savefig('/media/data/yanran/sax_1.jpg')
            #            plt.imshow(imgs[1,1,0,:,:].cpu(),cmap='gray')
            #            plt.savefig('/media/data/yanran/sax_2.jpg')
            #            plt.imshow(imgs[1,2,0,:,:].cpu(),cmap='gray')
            #            plt.savefig('/media/data/yanran/sax_3.jpg')
            #            ttt
            if (hasattr(self.backbone, 'features')
                ...
                #                plt.imshow(imgs[0,0,0,:,:].cpu(),cmap='gray')
                #                plt.savefig('/media/data/yanran/pad_1.jpg')
                #                plt.imshow(imgs[1,0,0,:,:].cpu(),cmap='gray')
                #                plt.savefig('/media/data/yanran/pad_2.jpg')
                #                plt.imshow(imgs[2,0,0,:,:].cpu(),cmap='gray')
                #                plt.savefig('/media/data/yanran/pad_3.jpg')
                #                tttt
...
#            plt.imshow(imgs[0,0,0,:,:].cpu(),cmap='gray')
#            plt.savefig('/media/data/yanran/sax_1.jpg')
#            plt.imshow(imgs[0,1,0,:,:].cpu(),cmap='gray')
#            plt.savefig('/media/data/yanran/sax_2.jpg')
#            plt.imshow(imgs[0,2,0,:,:].cpu(),cmap='gray')
#            plt.savefig('/media/data/yanran/sax_3.jpg')
#            ttt
            for i in range(len(self.weight)):
                imgs1 = imgs[i]
                model = self.backbone[i]
                if (hasattr(model, 'features') and self.backbone_from == 'torchvision'):
                    x1 = model.features(imgs1)
                elif self.backbone_from == 'timm':
                    x1 = model.forward_features(imgs1)
                else:
                    x1 = model(imgs1)
                x.append(x1)
    ...
    def forward(self, imgs=None, label=None, return_loss=True, sax=None, ch=None, lge=None, **kwargs):
        ...
        return self.forward_test(imgs, label, **kwargs)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        Returns:
            dict: It should contain at least 3 keys: ``loss``, ``log_vars``,
                ...
                averaging the logs.
        """
        if 'imgs' in data_batch:
            imgs = data_batch['imgs']
        else:
            imgs = list()
            if 'sax' in data_batch:
                sax = data_batch['sax']
                imgs.append(sax)
            if 'ch' in data_batch:
                ch = data_batch['ch']
                imgs.append(ch)
            if 'lge' in data_batch:
                lge = data_batch['lge']
                imgs.append(lge)
