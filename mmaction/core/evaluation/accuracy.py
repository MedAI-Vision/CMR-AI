...
def confusion_matrix(y_pred, y_real, normalize=None):
    ...
    np.save('Predicted_Value.npy',y_pred)
    np.save('True_Value.npy',y_real)
    print(f'Confusion matrix and Predicted/True Value are saved in {os.getcwd()}')
...
def mean_class_accuracy(scores, labels):
    ...
    evey_class_acc = [hit / cnt if cnt else 0.0 for cnt, hit in zip(cls_cnt, cls_hit)]

    
    eca='ECA.txt'
    with open(eca,'a+')as f:
        f.write(f'{evey_class_acc}\n')
    print(f"Every class ACC: {evey_class_acc}, saved into {os.getcwd()}\t{eca}")

    mean_class_acc = np.mean(evey_class_acc)
...
#    np.save('/media/data/yanran/final_outcome2/sax_4ch_bin_label.npy', labels)
#    print('save')
#    np.save('/media/data/yanran/final_outcome2/sax_4ch_bin_score.npy', scores)
    for k in topk:
...
def f1_score(scores, label):
    pred = np.argmax(scores, axis=1)
    score = f1(label,pred,average='macro')

#    cf_mat = confusion_matrix(pred, label).astype(float)
#
#    cls_cnt = cf_mat.sum(axis=1)  # 每一列的和向量
#    cls_hit = np.diag(cf_mat)  #对角线元素向量
#
#    arr = [hit / cnt if cnt else 0.0 for cnt, hit in zip(cls_cnt, cls_hit)]
#
#    tp1 = cls_cnt[0]*arr[0]
#    fp1 = cls_cnt[1]*(1-arr[1])
#    fn1 = cls_cnt[0]*(1-arr[0])
#    score = (2*tp1)/(2*tp1+fn1+fp1)
    return score
