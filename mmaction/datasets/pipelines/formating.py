...
class FormatShape:
    ...
    def __call__(self, results):
        ...
        if 'sax' in results:
            if not isinstance(results['sax'], np.ndarray):
                results['sax'] = np.array(results['sax'])
            imgs = results['sax']
            
            if self.collapse:
                assert results['num_clips_cine'] == 1

            if self.input_format == 'NCTHW':
                num_clips = results['num_clips_cine']
                clip_len = results['clip_len_cine']
            ...
            results['sax'] = imgs
            results['input_shape_sax'] = imgs.shape
        if 'ch' in results:
            if not isinstance(results['ch'], np.ndarray):
            ...
            results['ch'] = imgs
            results['input_shape_4ch'] = imgs.shape
        if 'lge' in results:
            if not isinstance(results['lge'], np.ndarray):
                results['lge'] = np.array(results['lge'])
            imgs = results['lge']
            
            if self.collapse:
                assert results['num_clips_lge'] == 1

            if self.input_format == 'NCTHW':
                num_clips = results['num_clips_lge']
                clip_len = results['clip_len_lge']
            ...
            results['lge'] = imgs
            results['input_shape_lge'] = imgs.shape
