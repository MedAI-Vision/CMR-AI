...
class Imgaug:
    ...
    def __call__(self, results):
        ...
        if 'sax' in results:
            in_type = results['sax'][0].dtype.type

            cur_aug = self.aug.to_deterministic()

            results['sax'] = [
                cur_aug.augment_image(frame) for frame in results['sax']
            ]
            img_h, img_w, _ = results['sax'][0].shape

            out_type = results['sax'][0].dtype.type
            assert in_type == out_type, \
                ('Imgaug input dtype and output dtype are not the same. ',
                 f'Convert from {in_type} to {out_type}')
        if 'ch' in results:
            in_type = results['ch'][0].dtype.type

#            cur_aug = self.aug.to_deterministic()
            import imgaug.augmenters as iaa
            cur_aug = iaa.Sequential([
                            iaa.Rotate((-20, 20))
                        ]).to_deterministic()

            results['ch'] = [
                cur_aug.augment_image(frame) for frame in results['ch']
            ]
            img_h, img_w, _ = results['ch'][0].shape

            out_type = results['ch'][0].dtype.type
            assert in_type == out_type, \
                ('Imgaug input dtype and output dtype are not the same. ',
                 f'Convert from {in_type} to {out_type}')
        if 'lge' in results:
            ...
            results['lge'] = [
                cur_aug.augment_image(frame) for frame in results['lge']
            ]
            img_h, img_w, _ = results['lge'][0].shape

            out_type = results['lge'][0].dtype.type
            assert in_type == out_type, \
                ('Imgaug input dtype and output dtype are not the same. ',
                 f'Convert from {in_type} to {out_type}')
...
class RandomErasing(tdata.random_erasing.RandomErasing):
    ...
    def __call__(self, results):
        ...
        for frame in results['imgs']:
            ...
            frame = super().__call__(torch.from_numpy(
                frame).permute(2, 0, 1)).permute(1, 2, 0).numpy()
...
class Resize:
    ...
    def __call__(self, results):
        ...
        if 'imgs' in results:
            ...
            results['scale_factor'] = results['scale_factor'] * \
                self.scale_factor
            ...
            if 'sax' in results:
                img_h, img_w = results['img_shape_sax']
                ...
                results['img_shape_sax'] = (new_h, new_w)
                if not self.lazy:
                    results['sax'] = self._resize_imgs(results['sax'], new_w,
                                                       new_h)
            if 'ch' in results:
                img_h, img_w = results['img_shape_4ch']
                ...
                results['img_shape_4ch'] = (new_h, new_w)
                if not self.lazy:
                    results['ch'] = self._resize_imgs(results['ch'], new_w,
                                                      new_h)
            if 'lge' in results:
                img_h, img_w = results['img_shape_lge']
                ...
                results['img_shape_lge'] = (new_h, new_w)
                if not self.lazy:
                    results['lge'] = self._resize_imgs(results['lge'], new_w,
                                                       new_h)
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        if modality == 'RGB':
            ...
            if 'sax' in results:
                n = len(results['sax'])
                h, w, c = results['sax'][0].shape
                imgs = np.empty((n, h, w, c), dtype=np.float32)
                for i, img in enumerate(results['sax']):
                    imgs[i] = img

                for img in imgs:
                    mmcv.imnormalize_(img, np.array([self.mean[0], self.mean[0], self.mean[0]], dtype=np.float32), np.array(
                        [self.std[0], self.std[0], self.std[0]], dtype=np.float32), self.to_bgr)

                results['sax'] = imgs
                results['sax_norm_cfg'] = dict(
                    mean=[self.mean[0], self.mean[0], self.mean[0]], std=[self.std[0], self.std[0], self.std[0]], to_bgr=self.to_bgr)
            if 'ch' in results:
                n = len(results['ch'])
                h, w, c = results['ch'][0].shape
                imgs = np.empty((n, h, w, c), dtype=np.float32)
                for i, img in enumerate(results['ch']):
                    imgs[i] = img

                for img in imgs:
                    mmcv.imnormalize_(img, np.array([self.mean[1], self.mean[1], self.mean[1]], dtype=np.float32), np.array(
                        [self.std[1], self.std[1], self.std[1]], dtype=np.float32), self.to_bgr)

                results['ch'] = imgs
                results['4ch_norm_cfg'] = dict(
                    mean=[self.mean[1], self.mean[1], self.mean[1]], std=[self.std[1], self.std[1], self.std[1]], to_bgr=self.to_bgr)
            if 'lge' in results:
                n = len(results['lge'])
                h, w, c = results['lge'][0].shape
                imgs = np.empty((n, h, w, c), dtype=np.float32)
                for i, img in enumerate(results['lge']):
                    imgs[i] = img

                for img in imgs:
                    mmcv.imnormalize_(img, np.array([self.mean[2], self.mean[2], self.mean[2]], dtype=np.float32), np.array(
                        [self.std[2], self.std[2], self.std[2]], dtype=np.float32), self.to_bgr)

                results['lge'] = imgs
                results['lge_norm_cfg'] = dict(
                    mean=[self.mean[2], self.mean[2], self.mean[2]], std=[self.std[2], self.std[2], self.std[2]], to_bgr=self.to_bgr)
            return results
        if modality == 'Flow':
...
class ColorJitter:
    ...
    def __call__(self, results):
        ...
        if 'lge' in results:
            ...
            results['lge'] = [img + rgb for img in out]
            results['eig_val'] = self.eig_val
            results['eig_vec'] = self.eig_vec
            results['alpha_std'] = self.alpha_std
            results['color_space_aug'] = self.color_space_aug
        ...
        if self.cine:
            if 'sax' in results:
                ...
                results['sax'] = [img + rgb for img in out]
                results['eig_val'] = self.eig_val
                results['eig_vec'] = self.eig_vec
                results['alpha_std'] = self.alpha_std
                results['color_space_aug'] = self.color_space_aug
            ...
            if 'ch' in results:
                ...
                results['ch'] = [img + rgb for img in out]
                results['eig_val'] = self.eig_val
                results['eig_vec'] = self.eig_vec
                results['alpha_std'] = self.alpha_std
                results['color_space_aug'] = self.color_space_aug
...
class Padding:
    ...
    def pad(self, imgs):
        ...
        for img in imgs:
            x = img.shape[0]
            y = img.shape[1]
            pad_x1 = (self.size[0]-x)//2
            pad_x2 = self.size[0]-x-pad_x1
            pad_y1 = (self.size[1]-y)//2
            pad_y2 = self.size[1]-y-pad_y1
            new_img = np.zeros((self.size[0], self.size[1], img.shape[-1]))
        ...
        return np.float32(new_imgs)

    def __call__(self, results):
        results['img_shape'] = self.size
        if 'imgs' in results:
            results['imgs'] = self.pad(results['imgs'])
            results['img_shape'] = self.size
        if 'sax' in results:
            results['sax'] = self.pad(results['sax'])
            results['img_shape_sax'] = self.size
        if 'ch' in results:
            results['ch'] = self.pad(results['ch'])
            results['img_shape_4ch'] = self.size

        return results


@PIPELINES.register_module()
class AddRandomNumber:
    def __init__(self, range):
        self.range = range

    def add(self, imgs):
        for i in range(len(imgs)):
            num = np.random.uniform(self.range[0], self.range[1])
            imgs[i] = imgs[i] + num
        return np.float32(imgs)

    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.add(results['imgs'])
        if 'sax' in results:
            results['sax'] = self.add(results['sax'])
        if 'ch' in results:
            results['ch'] = self.add(results['ch'])
        return results
...
class HistEqual:
    ...
    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.__hist(results['imgs'])
        if 'sax' in results:
            results['sax'] = self.__hist(results['sax'])
        if 'ch' in results:
            results['ch'] = self.__hist(results['ch'])
        return results


@PIPELINES.register_module()
class RandomShift:
    def __init__(self, shift=None, ratio=0.3):
        self.ratio = ratio
        self.shift = shift

    def __shift(self, imgs):
        for i in range(len(imgs)):
            img = imgs[i]
            img = np.array(img)
            if np.random.rand() < self.ratio:
                if self.shift is None:
                    self.shift = 0.1 * \
                        np.array(img.shape[:2])*(2*np.random.rand()-1)
                img = cv2.warpAffine(src=img, M=np.float32([[1, 0, self.shift[0]], [
                                     0, 1, self.shift[1]]]), dsize=(img.shape[1], img.shape[0]))
            img = img.astype(np.uint8)
            imgs[i] = img
        return np.float32(imgs)
    ...
    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.__shift(results['imgs'])
        if 'sax' in results:
            results['sax'] = self.__shift(results['sax'])
        if 'ch' in results:
            results['ch'] = self.__shift(results['ch'])
        return results
...
class SingleNorm:
    ...
    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.__norm(results['imgs'])
        if 'sax' in results:
...
class Random_Gamma_Bright:
    ...
    def __adjust(self, imgs):
        for i in range(len(imgs)):
            if np.random.rand() < self.ratio:
                imgs[i] = self.__norm(exposure.adjust_gamma(
                    imgs[i], np.random.uniform(0.4, 1)))
        return np.float32(imgs)

    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.__adjust(results['imgs'])
        if 'sax' in results:
            results['sax'] = self.__adjust(results['sax'])
        if 'ch' in results:
            results['ch'] = self.__adjust(results['ch'])
        return results
...
class Random_Noise:
    ...
    def random_noise(self, image, noise_num=200):
        '''
        random noise
        args:
            image: img need to be operated
            noise_num: number of noise points
        return: image operated
        '''
        img_noise = image
        if np.random.rand() < self.ratio:
            ...
            for i in range(noise_num):
                ...
                img_noise[x, y, :] = 255
        return np.float32(img_noise)

    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.random_noise(results['imgs'])
        if 'sax' in results:
            results['sax'] = self.random_noise(results['sax'])
        if 'ch' in results:
            results['ch'] = self.random_noise(results['ch'])
        return results
...
class Reverse_Phase:
    ...
    def __reverse_phase(self, imgs):
        '''
        reverse phase
        args:
            imgs: img need to be operated
        return: image operated
        '''
        for i in range(len(imgs)):
            imgs[i] = 255.0-self.__norm(imgs[i])
        return np.float32(imgs)

    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.__reverse_phase(results['imgs'])
        if 'sax' in results:
            results['sax'] = self.__reverse_phase(results['sax'])
        if 'ch' in results:
            results['ch'] = self.__reverse_phase(results['ch'])
        return results
...
class Range_Norm:
    ...
    def __range_norm(self, imgs):
        for i in range(len(imgs)):
            imgs[i] = self.__norm(imgs[i])
        return np.float32(imgs)

    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.__range_norm(results['imgs'])
        if 'sax' in results:
            results['sax'] = self.__range_norm(results['sax'])
        if 'ch' in results:
            results['ch'] = self.__range_norm(results['ch'])
        return results


@PIPELINES.register_module()
class Random_rotate:
    def __init__(self, range=30, ratio=0.3):
        self.range = range
        self.ratio = ratio
    ...
    def rotate(self, imgs):
        if np.random.rand() < self.ratio:
            if len(imgs[0].shape) == 3:
                rows, cols = imgs[0][:, :, 0].shape
            else:
                rows, cols = imgs[0].shape
            Matrix = cv2.getRotationMatrix2D(
                (cols/2, rows/2), (2*np.random.rand()-1)*self.range, 1)
            for i in range(len(imgs)):
                imgs[i] = cv2.warpAffine(
                    imgs[i], Matrix, (rows, cols), borderValue=(255, 255, 255))
        return np.float32(imgs)

    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.rotate(results['imgs'])
        if 'sax' in results:
            results['sax'] = self.rotate(results['sax'])
        if 'ch' in results:
            results['ch'] = self.rotate(results['ch'])
        return results
...
class Flip_Z:
    ...
    def flip_z(self, imgs):
        if np.random.rand() < self.ratio:
            return np.ascontiguousarray(np.float32(np.flip(imgs, axis=0)))
        else:
            return np.float32(imgs)

    def __call__(self, results):
        if 'imgs' in results:
            results['imgs'] = self.flip_z(results['imgs'])
        # if 'sax' in results:
